...
class ZhihuSpider(Spider):
    ...
    def parse_user(self, response):
        ...
        yield Request(self.follows_url.format(user=result.get('url_token'), include=self.follows_query, limit=20, offset=0),self.parse_follows) # 将粉丝的request加入队列
        yield Request(self.followers_url.format(user=result.get('url_token'), include=self.followers_query, limit=20, offset=0),self.parse_followers) # 将关注的人的Request加入队列
